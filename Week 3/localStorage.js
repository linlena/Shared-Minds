...
// lets anyone store things in the browser 
function store() {
    console.log("store");
...
function init3D() {
    ...
    let panotexture = new THREE.TextureLoader().load("sky.jpeg");
...
function initHTML() {
    ...
    function deleteSelectedObject() {
        if (selectedObject) {
            ...
            clickableMeshes = clickableMeshes.filter(mesh => mesh !== selectedObject.mesh);
            delete myObjectsByThreeID[selectedObject.threeID];
            delete storageJSON[selectedObject.threeID];
            store();
